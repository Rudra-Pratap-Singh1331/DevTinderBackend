...
connectionRouter.post("/request/review/:typeOfReq/:objId", userAuthMiddleware , async (req, res)=>{
  ...
  try{
    ...
    if(!allowedStatus.includes(req.params.typeOfReq)) return res.status(404).json({
      status:false,
      error:{
    ...
    if(!mongoose.Types.ObjectId.isValid(req.params.objId)) return res.status(404).json({
      status:false,
      error:{
    ...
    if(!connectionReq) return res.status(404).json({
      status:false,
      error:{
